...
| `byText.ts` | `.sort()`'s own order as an explicit comparator. Internal — imported by the three modules that sort and NOT re-exported from `index.ts`. It changes no order; it exists so the lint rule asking for a comparator (which cannot tell a string array from a number one) is answered without one. The orders it preserves are published documents and refusal lines, so `test/test_byText.ts` pins the equivalence rather than trusting it. |
